...
if grep -q '^app.version=' "$APP_DIR/application.properties" 2>/dev/null; then
  sed -i "s/^app.version=.*/app.version=${TAG}/" "$APP_DIR/application.properties"
else
  echo "app.version=${TAG}" >> "$APP_DIR/application.properties"
fi
systemctl restart ruomi
sleep 25
curl -s -o /dev/null -w "HTTP %{http_code}\n" http://127.0.0.1:8080/
curl -s http://127.0.0.1:8080/ | grep -c 'admin-theme.css' || true
