...
grep -q '^app.timezone=' "$APP_DIR/application.properties" 2>/dev/null && \
  sed -i 's/^app.timezone=.*/app.timezone=Europe\/Helsinki/' "$APP_DIR/application.properties" || \
  echo 'app.timezone=Europe/Helsinki' >> "$APP_DIR/application.properties"
timedatectl set-timezone Europe/Helsinki 2>/dev/null || ln -sf /usr/share/zoneinfo/Europe/Helsinki /etc/localtime
mkdir -p /etc/systemd/system/ruomi.service.d
cat > /etc/systemd/system/ruomi.service.d/timezone.conf << 'EOF'
[Service]
Environment="JAVA_TOOL_OPTIONS=-Duser.timezone=Europe/Helsinki"
EOF
systemctl daemon-reload
systemctl restart ruomi
sleep 25
curl -s -o /dev/null -w "HTTP %{http_code}\n" http://127.0.0.1:8080/
mysql -u ruomi -p'Hjvfynbr1221' ruomi -e "SELECT version, success FROM flyway_schema_history ORDER BY installed_rank DESC LIMIT 3;" 2>/dev/null || true
rm -rf "$TMP"
echo "Deploy ${TAG} done (timezone Europe/Helsinki)"
